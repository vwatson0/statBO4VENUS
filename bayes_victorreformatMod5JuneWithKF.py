...
def black_box_function(settingsN):
    ...
    balzer1val = setbalzer(1,balzer1,7.4,7.5)
    ...
    v_list = []
    KF = KFparAlt(10000.,1.) # Mod vwatson KF June5 | setting up Kalman object
    ...
    if killoff == 0:
        ...
        #for j in range(30):
            ...
            if len(v_list) > MinSample2Move: # Mod vwatson KF June5 | waited enougth to test the slope ?
                if (np.abs(slope[len(slope)-NsampleTestSlope-1:len(slope)-1]) < SettleSlope).all:# Mod vwatson KF June5 | is abs(slope) small enough (check code)
...
def setnumbers(balzer1N,biasN):
    scale_balzer1 = [7.6,8.6,balzer1N]
