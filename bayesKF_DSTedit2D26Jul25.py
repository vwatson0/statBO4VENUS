...
#dst                settled = True # Mod vwatson KF June5
    ...
    print(f'{announce} {balzer1:5.2f} + {bias:6.2f}: {v_mean:7.2f} {rel_std*100:6.2f}% tstep={time.time()-tstartwatch:.2f}')
...
succeedname = 'data/succeed_'+str(int(tstart))
...
for cptr in range(maxIter):
    ...
    gpr = GaussianProcessRegressor(kernel=kernel, random_state=0, alpha = 1.e-5)
    # Fitting the data
    gpr.fit(Xdata, Ydata)

    # Initializing the standard optimizer for Acq function
    # Modification 5 June 2025 start | vwatson ###########################################################
    MaxIterForAcqOptInit = 2000 # number max of iterations to ind an area where the acquisition function is computable
